...
export default function ManagePortfolio() {
  ...
  const submitHandler = async ({ mode, item, name, token }) => {
    const url =
      mode === "add" ? APPURL.portfolio : `${APPURL.portfolio}${item?.id}/`;
  ...
  const {
    ...
    mutationFn: ({ mode, item, name }) =>
      submitHandler({ mode, item, name, token: LocalData?.token }),
    onSuccess: (data) => {
  ...
  const {
    ...
    onError: (err) => {
      toast.error(err.message || "Error deleting portfolio");
  ...
  return (
    <>
      ...
      <ReusableTable data={portfolio} columns={columns} />
      ...
      >
        <div className="bg-stone-900 max-w-md w-full rounded-lg p-6 text-white">
          <h2 className="text-xl font-semibold mb-4">
            Confirm Deletion - {deleteData.data?.name}
          </h2>
          <p className="mb-6">
          ...
          <div className="flex justify-end gap-4">
            ...
            <button
              onClick={() => {
                deletePortfolio({ id: deleteData?.data?.id });
              }}
